...
class DataService {
  ...
  cleanCorruptedLocalData(): void {
    try {
      const rawIndex = this.getStoredData<any[]>(STORAGE_KEYS.CAMPAIGN_INDEX) || [];
      const cleanIndex: CampaignSummary[] = rawIndex
        .filter((item) => typeof item?.id === 'string' && item.id)
        .map((item) => ({
          id: item.id,
          name: typeof item?.name === 'string' && item.name ? item.name : '未命名模组',
      ...
      this.saveDataToStorage(STORAGE_KEYS.CAMPAIGN_INDEX, cleanIndex);

      cleanIndex.forEach((summary) => {
        const key = `trpg_campaign_${summary.id}`;
        const rawCampaign = this.getStoredData<any>(key);
        if (!rawCampaign) return;
        const normalized = this.normalizeCampaignData(rawCampaign);
        this.saveDataToStorage(key, normalized);
      });

      const legacy = this.getStoredData<any>(STORAGE_KEYS.CAMPAIGN_DATA);
      if (legacy) {
        this.saveDataToStorage(STORAGE_KEYS.CAMPAIGN_DATA, this.normalizeCampaignData(legacy));
      }

      this.storage.keys()
        .filter((key) => key.startsWith(STORAGE_KEYS.DRAFT_PREFIX))
        .forEach((draftKey) => {
          const rawDraft = this.getStoredData<any>(draftKey);
          if (!rawDraft) return;
          this.saveDataToStorage(draftKey, this.normalizeCampaignData(rawDraft));
        });
    } catch (e) {
      console.error('Error cleaning corrupted local data', e);
    }
  }
  ...
  getCampaigns(userId?: string): CampaignSummary[] {
    const all = this.getStoredData<CampaignSummary[]>(STORAGE_KEYS.CAMPAIGN_INDEX) || [];
  ...
  createCampaign(name: string, description: string, ownerId: string): CampaignData {
    ...
    // Save actual data
    this.saveDataToStorage(`trpg_campaign_${newId}`, newCampaign);
    ...
    this.saveDataToStorage(STORAGE_KEYS.CAMPAIGN_INDEX, index);
  ...
  loadCampaign(id?: string): CampaignData {
    if (!id) {
      // Fallback for legacy single-campaign mode or direct access
      // Try to load legacy data first
      const legacy = this.getStoredData<any>(STORAGE_KEYS.CAMPAIGN_DATA);
      if (legacy) return this.normalizeCampaignData(legacy);
      return DEFAULT_CAMPAIGN_DATA;
    }

    const data = this.getStoredData<any>(`trpg_campaign_${id}`);
    if (data) {
      const normalized = this.normalizeCampaignData(data);
      this.saveDataToStorage(`trpg_campaign_${normalized.id}`, normalized);
      return normalized;
    }
    const fresh = this.normalizeCampaignData({ ...DEFAULT_CAMPAIGN_DATA, id });
    this.saveDataToStorage(`trpg_campaign_${fresh.id}`, fresh);
    return fresh;
  }
  ...
  saveCampaign(data: CampaignData): void {
    ...
    if (updatedData.id) {
      ...
      this.saveDataToStorage(`trpg_campaign_${updatedData.id}`, updatedData);
      ...
      // Legacy Save
      this.saveDataToStorage(STORAGE_KEYS.CAMPAIGN_DATA, updatedData);
    }
  }

  deleteCampaign(id: string): void {
    this.storage.removeItem(`trpg_campaign_${id}`);
    const index = this.getCampaigns();
    const newIndex = index.filter(c => c.id !== id);
    this.saveDataToStorage(STORAGE_KEYS.CAMPAIGN_INDEX, newIndex);
  }

  // --- Drafts ---
  hasUnsavedDraft(campaignId: string = 'default'): boolean {
    const key = `${STORAGE_KEYS.DRAFT_PREFIX}${campaignId}`;
  ...
  saveDraft(data: CampaignData): void {
    ...
    this.saveDataToStorage(`${STORAGE_KEYS.DRAFT_PREFIX}${id}`, draftData);
  }

  loadDraft(campaignId: string = 'default'): CampaignData | null {
    return this.getStoredData<CampaignData>(`${STORAGE_KEYS.DRAFT_PREFIX}${campaignId}`);
  }

  discardDraft(campaignId: string = 'default'): void {
    this.storage.removeItem(`${STORAGE_KEYS.DRAFT_PREFIX}${campaignId}`);
  }
